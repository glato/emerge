            ...
            try:
                ...
                for run in range(optimization_runs):
                    ...
                    if run == optimization_runs - 1:
                        for node_name in partition_by_louvain:
                            if node_name in results:
                                result = results[node_name]
                                if isinstance(result, AbstractFileResult):
                                    result.metrics[self.Keys.LOUVAIN_MODULARITY_IN_FILE.value] = partition_by_louvain[node_name]
                                    self.local_data[result.unique_name] = {self.Keys.LOUVAIN_MODULARITY_IN_FILE.value: partition_by_louvain[node_name]}
                                if isinstance(result, AbstractEntityResult):
                                    result.metrics[self.Keys.LOUVAIN_MODULARITY_IN_ENTITY.value] = partition_by_louvain[node_name]
                                    self.local_data[result.unique_name] = {self.Keys.LOUVAIN_MODULARITY_IN_ENTITY.value: partition_by_louvain[node_name]}
                                ...
                                if graph_instance.graph_type == GraphType.ENTITY_RESULT_DEPENDENCY_GRAPH or \
                                    ...
                                    self.local_data[node_name] = {self.Keys.LOUVAIN_MODULARITY_IN_ENTITY.value: partition_by_louvain[node_name]}
                                if graph_instance.graph_type == GraphType.FILE_RESULT_DEPENDENCY_GRAPH:
                                    self.local_data[node_name] = {self.Keys.LOUVAIN_MODULARITY_IN_FILE.value: partition_by_louvain[node_name]}
