...
class RubyParser(AbstractParser, AbstractParsingCore):
    ...
    def generate_file_result_from_analysis(self, analysis, *, file_name: str, file_content: str) -> None:
        ...
        file_result = FileResult.create_file_result(analysis, file_name, "", self.parser_name(), LanguageType.RUBY, scanned_tokens)
    ...
    def _add_imports_to_result(self, result: AbstractResult):
        ...
        for _, obj, following in self._gen_word_read_ahead(filtered_list_no_comments):
            if obj == RubyParsingKeyword.REQUIRE.value:
                ...
                LOGGER.debug(f'require found: {getattr(parsing_result, CoreParsingKeyword.IMPORT_ENTITY_NAME.value)} and added to result')
