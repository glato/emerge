...
from tests.testdata.typescript import TYPESCRIPT_TEST_FILES, TYPESCRIPT_ENTITY_TEST_FILES
...
class TypeScriptParserTestCase(unittest.TestCase):
    ...
    def test_generate_entity_results(self):
        """Generate entity results for TypeScript classes and check their basic attributes and inheritance."""
        self.assertFalse(self.parser.results)

        for file_name, file_content in TYPESCRIPT_ENTITY_TEST_FILES.items():
            self.parser.generate_file_result_from_analysis(self.analysis, file_name=file_name, full_file_path="/tests/" + file_name, file_content=file_content)
